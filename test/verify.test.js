...
test.before("initialize a existing lottery contract", async t => {
	...
	//var lotteryContract = deploy.lotteryAbi;//web3.eth.contract(jsonInterface);
	await txnUtils.retryPromise(
		() => {
			return deploy.lottery.getEscrow;
		},
		30);
	var lottery = deploy.lottery;
	//var lottery = lotteryContract.at(lotteryAddr);
